    ...
    def example_images(self,img,start_token,vocabulary,device,max_length=50,dim_model=64):
        ...
        with torch.no_grad():
            ...
            for _ in range(max_length):
                caption= self.decoder.embedding(y_input)* math.sqrt(self.dim_model)
                caption = self.decoder.positional_encoder(caption)
                caption = caption.permute(1, 0, 2)

                transformer_out = self.decoder.TransformerDecoder(caption,x)
                predicted = self.decoder.out(transformer_out[:,-1])
                _,next_word = torch.max(predicted,dim=1)
                
                #pred = self.decoder(x,y_input)
                
                next_item = predicted.topk(1)[1].view(-1)[-1].item()
                #next_item = predicted.argmax(2)[:,-1].item()
