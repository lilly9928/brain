...
def make_vocab(input_dir,output_dir):
    ...
    caption_length = []
    captions = pd.read_excel(input_dir, usecols=[1])
    set_caption_length = [None] * len(captions)

    for i in range(len(captions['str'])):
        word_len = len(captions['str'].values[i].split('\n'))
        sentence = captions['str'].values[i].split('\n')
...
if __name__ == '__main__':
    input_dir = 'D:/data/brain/sentence_data.xlsx'
    output_dir ='D:/data/brain'
